### 2026 EUC
和塔神vp写了6个，去现场夺冠了。
A 是个 n = 40 然后 n^6 的东西，感觉不太是题。
I 的推导可以回头再看看，感觉如果周期引理用的熟练的话会推的更快一点。J 回头再看看，一开始是哪里没捋顺。K这个multiset 任意系数 union 后期望是凸组合的结论可以记一下，总不能每次都靠感受出来。

### cf 2176
连 div2 都能掉分了，还要脸吗。
回头巩固一下枚举gcd的时候怎么用莫反去处理。

### The 2025 ICPC Asia Seoul Regional Contest
5h 8题，主要是低级错误不断。极角排序后双指针写了特别久结果写了个会退化成 n^2 的东西，换成二分一下就过了。双指针写不明白且常数允许当然要考虑二分。
待补。

### abc435
25min A-F。G是个容斥推出来以后按%4分类讨论然后前缀和处理,不应该整场不会做。

### cf2174
牛逼！div1怒做一题。

B 是个需要贪心先扔掉把n规模变小再dp（需要前缀优化的东西），因为感觉要前缀优化的dp不是很容易，不像div1b的难度，写了个更弱的dp（这个更弱的dp是错的），交上去wa了以为是前面的贪心错了，然后就爆掉了一整把，唐得没边了。

C 猜到了需要猜的结论，但是不太会处理 E(X^2), 这里考虑 X = x_1 + x_2 ... x_n, 那么 E(X^2) = E(\sum xi * xj), 然后如果任意xi xj是独立的，就可以拆成E(xi) * E(xj)。

F 是EGF，虽然没有那么难，但是过那么多应该是gpt大人发力了。
